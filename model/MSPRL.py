...
class SFE(nn.Module):
    def __init__(self, image_channel, channel):
        ...
        planes = image_channel + channel * 2
        self.enconv = nn.Sequential(
            nn.Conv2d(image_channel, channel, kernel_size=3, padding=1, stride=1),
            nn.Conv2d(channel, channel, kernel_size=3, padding=1, stride=1, groups=channel),
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(planes, planes, kernel_size=3, padding=1, stride=1, groups=planes, bias=False),
            nn.Conv2d(planes, channel, kernel_size=1, padding=0, stride=1, bias=False)
        )

    def forward(self, x, y):
        res = self.enconv(x)
        res = res * y
        res = torch.cat([x, res, y], dim=1)
        res = self.conv(res)
        res = res + y
        return res
...
if __name__ == '__main__':
    ...
    # print(net)
    y = net(torch.randn(16, 1, 256, 256))
    print(y.size())
