...
def train(args):
    ...
    while current_iter < total_iters:
        ...
        for batch_id, (predict, target) in enumerate(train_loader):
            ...
            output_map_fft = torch.stack((output_map_fft.real, output_map_fft.imag), -1)
        ...
        if epoch % save_freq == 0:
            ...
            save_model(checkpoint, model_save_dir, model_name, total_iters, mode='resume')
        ...
        if epoch % valid_freq == 0:
            ...
            if state['avg_psnr'] > state['max_psnr']:
                ...
                save_model(net.state_dict(), model_save_dir, model_name, total_iters, mode='net')
...
def valid(net, device, valid_loader, valid_data_len, state, l1_criterion, fft_loss_factor):
    ...
    test_avg_loss = test_loss / valid_data_len

    avg_psnr = epoch_psnr / valid_data_len
    state['test_avg_loss'] = test_avg_loss
...
def save_model(net, model_save_dir, model_name, total_iters, mode):
    if mode == 'resume':
        model_save_dir = '{}/resume_{}_{}.pth'.format(model_save_dir, model_name, total_iters)
    else:
        model_save_dir = '{}/{}_{}.pth'.format(model_save_dir, model_name, total_iters)
